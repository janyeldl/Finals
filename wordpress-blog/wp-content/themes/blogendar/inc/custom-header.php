 ...
 *
 * @uses blogendar_header_style()
 */
function blogendar_custom_header_setup() {
	add_theme_support( 'custom-header', 
		apply_filters( 'blogendar_custom_header_args', 
			array(
				...
				'height'                 => 528,
				'flex-height'            => true,
				'wp-head-callback'       => 'blogendar_header_style',
			)
		)
	);
	...
	register_default_headers( array(
		'default-image' => array(
			...
			'description'   => esc_html__( 'Default Header Image', 'blogendar' ),
		),
	) );
}
add_action( 'after_setup_theme', 'blogendar_custom_header_setup' );

if ( ! function_exists( 'blogendar_header_style' ) ) :
	/**
	 * Styles the header image and text displayed on the blog.
	 *
	 * @see blogendar_custom_header_setup().
	 */
	function blogendar_header_style() {
		...
		wp_add_inline_style( 'blogendar-style', $css );
	}
endif;
add_action( 'wp_enqueue_scripts', 'blogendar_header_style', 10 );