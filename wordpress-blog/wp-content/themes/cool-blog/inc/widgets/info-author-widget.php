<?php
if ( ! class_exists( 'Cool_Blog_Author_Info_Widget' ) ) {
	/**
	 * Adds Cool Blog Author Info Widget.
	 */
	class Cool_Blog_Author_Info_Widget extends WP_Widget {

		/**
		 * Register widget with WordPress.
		 */
		public function __construct() {
			$cool_blog_author_info_widget = array(
				'classname'   => 'widget adore-widget author-widget',
				'description' => __( 'Retrive Author Information Widget', 'cool-blog' ),
			);
			parent::__construct(
				'cool_blog_author_info_widget',
				__( 'Cool Blog: Author Info Widget', 'cool-blog' ),
				$cool_blog_author_info_widget
			);
		}
		...
		public function form( $instance ) {
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'name' ) ); ?>"><?php esc_html_e( 'Author Name:', 'cool-blog' ); ?></label>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'description' ) ); ?>"><?php esc_html_e( 'Description :', 'cool-blog' ); ?></label>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'author_image_url' ) ); ?>"><?php esc_html_e( 'Author Image URL', 'cool-blog' ); ?></label>:<br />
				...
				<input type="button" class="select-img button" value="<?php esc_attr_e( 'Upload', 'cool-blog' ); ?>" data-uploader_title="<?php esc_attr_e( 'Select Image', 'cool-blog' ); ?>" data-uploader_button_text="<?php esc_attr_e( 'Choose Image', 'cool-blog' ); ?>" style="margin-top:5px;" /><br/>
			</p>
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'social_link_number' ) ); ?>"><?php esc_html_e( 'Number of links to show:', 'cool-blog' ); ?></label>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'open_link_new_tab' ) ); ?>"><?php esc_html_e( 'Open Social link in New Tab', 'cool-blog' ); ?>:</label>
				<input type="checkbox" id="<?php echo esc_attr( $this->get_field_id( 'open_link_new_tab' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'open_link_new_tab' ), 'cool-blog' ); ?>"  <?php checked( $open_link_new_tab, true ); ?> />
			</p>
			<?php
			for ( $i = 1; $i <= $social_link_number; $i++ ) {
				...
				?>
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'link' . '-' . $i ) ); ?>"><?php sprintf( esc_html__( 'Link %s :', 'cool-blog' ), $i ); ?></label>
		...
		public function update( $new_instance, $old_instance ) {
			...
			$instance['open_link_new_tab']  = cool_blog_sanitize_checkbox( $new_instance['open_link_new_tab'] );
 ...
 * @since Cool Blog 1.0
 **/
function cool_blog_author_info_widget_image_upload_enqueue( $hook ) {
	...
	wp_enqueue_media();
	wp_enqueue_script( 'author-info-widget-image-upload-script', get_template_directory_uri() . '/assets/js/upload.js', array( 'jquery' ), COOL_BLOG_VERSION, true );
}

add_action( 'admin_enqueue_scripts', 'cool_blog_author_info_widget_image_upload_enqueue' );
